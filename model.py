...
class SingleViewto3D(nn.Module):
    def __init__(self, args):
        ...
        if args.type == "vox":
            ...
            self.decoder= torch.nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(in_features=1024, out_features=32768),
            ...
            #     nn.ReLU(),
            #     nn.Linear(in_features=2048, out_features=3*self.n_point),
            #     )
            self.decoder = torch.nn.Sequential(
            ...
            self.decoder = torch.nn.Sequential(
                ...
                nn.Linear(in_features=1024, out_features=2048),
                nn.ReLU(),
                nn.Linear(in_features=2048, out_features=mesh_pred.verts_packed().shape[0]*3),
                nn.Tanh(),
                )
    ...
    def forward(self, images, args):
        ...
        if args.type == "vox":
            # TODO:
            voxels_pred_ = self.decoder(encoded_feat)
            voxels_pred = voxels_pred_.view(-1, 1, 32, 32, 32)
            return voxels_pred

        elif args.type == "point":
            # TODO:
            pointclouds_pred_ = self.decoder(encoded_feat)
            pointclouds_pred = pointclouds_pred_.view(-1, self.n_point, 3)
