...
def evaluate_model(args):
    ...
    for step in range(start_iter, max_iter):
        ...
        # TODO:
        if (step % args.vis_freq) >= 1 or (step % args.vis_freq) <= 3:
            # visualization block

            if args.type == "vox":
                images_gt = images_gt.squeeze(0).cpu().numpy()
                plt.imsave(f"results/Prob_2_1_vox/{args.type}_{step}_rgb_img.jpg", images_gt)
                render_vox_360(predictions, output_path=f"results/Prob_2_1_vox/{args.type}_{step}_prediction.gif",device=args.device)
                voxel_ground_truth = feed_dict['voxels'].to(args.device)
                render_vox_360(voxel_ground_truth, output_path=f"results/Prob_2_1_vox/{args.type}_{step}_ground_truth.gif",device=args.device)

            elif args.type == "point":
                images_gt = images_gt.squeeze(0).cpu().numpy()
                plt.imsave(f"results/Prob_2_2_pc/{args.type}_{step}_rgb_img.jpg", images_gt)
                render_points_360(predictions[0].unsqueeze(0), output_path=f"results/Prob_2_2_pc/{args.type}_{step}_prediction.gif",device=args.device)
                gt_points = sample_points_from_meshes(mesh_gt, args.n_points).to(args.device)
                render_points_360(gt_points, output_path=f"results/Prob_2_2_pc/{args.type}_{step}_ground_truth.gif",device=args.device)
            elif args.type == "mesh":
                images_gt = images_gt.squeeze(0).cpu().numpy()
                plt.imsave(f"results/Prob_2_3_mesh/{args.type}_{step}_rgb_img.jpg", images_gt)
                render_mesh_360(predictions[0], output_path=f"results/Prob_2_3_mesh/{args.type}_{step}_prediction.gif",device=args.device)
                render_mesh_360(mesh_gt, output_path=f"results/Prob_2_3_mesh/{args.type}_{step}_ground_truth.gif",device=args.device)
